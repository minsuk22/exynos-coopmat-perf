...
V7SRC="$(dirname "$0")/shaders/gemm_v07.comp"
V6SRC="$(dirname "$0")/shaders/fma_v06.comp"
FPSRC="$(dirname "$0")/shaders/fma_peak.comp"
GSSRC="$(dirname "$0")/shaders/gemm_tiled_scalar.comp"
...
# v07-style WMMA GEMM (block + warp tiled, padded shared).
"$GLSLANG" $TENV -DA_TYPE=float16_t -DC_TYPE=float32_t -V "$V7SRC" -o "$OUT/gemm_v07_fp16_fp32.spv"
"$GLSLANG" $TENV -DA_TYPE=float16_t -DC_TYPE=float16_t -V "$V7SRC" -o "$OUT/gemm_v07_fp16_fp16.spv"
"$GLSLANG" $TENV -DA_TYPE=int8_t    -DC_TYPE=int32_t   -V "$V7SRC" -o "$OUT/gemm_v07_s8_s32.spv"
"$GLSLANG" $TENV -DA_TYPE=uint8_t   -DC_TYPE=uint32_t  -V "$V7SRC" -o "$OUT/gemm_v07_u8_u32.spv"
# v06-style FMA GEMM (warp/thread tiled, transposed A, register blocking).
"$GLSLANG" $TENV -DA_TYPE=float16_t -DC_TYPE=float32_t -V "$V6SRC" -o "$OUT/fma_v06_fp16_fp32.spv"
"$GLSLANG" $TENV -DA_TYPE=float16_t -DC_TYPE=float16_t -V "$V6SRC" -o "$OUT/fma_v06_fp16_fp16.spv"
"$GLSLANG" $TENV -DA_TYPE=int8_t    -DC_TYPE=int32_t   -V "$V6SRC" -o "$OUT/fma_v06_s8_s32.spv"
"$GLSLANG" $TENV -DA_TYPE=uint8_t   -DC_TYPE=uint32_t  -V "$V6SRC" -o "$OUT/fma_v06_u8_u32.spv"
# Pure FMA/MAD peak microbenchmark (ALU counterpart of wmma_peak).
"$GLSLANG" $TENV -DC_TYPE=float32_t -DVACC=vec4              -V "$FPSRC" -o "$OUT/fma_peak_fp16_fp32.spv"
"$GLSLANG" $TENV -DC_TYPE=float16_t -DVACC=f16vec4           -V "$FPSRC" -o "$OUT/fma_peak_fp16_fp16.spv"
"$GLSLANG" $TENV -DC_TYPE=int32_t   -DVACC=i32vec4 -DIS_INT=1 -V "$FPSRC" -o "$OUT/fma_peak_s8_s32.spv"
# Scalar shared-memory tiled GEMM (CUDA port, no WMMA), block-size sweep.
"$GLSLANG" $TENV -DA_TYPE=float16_t -DC_TYPE=float32_t -V "$GSSRC" -o "$OUT/gemm_scalar_fp16_fp32.spv"
"$GLSLANG" $TENV -DA_TYPE=float16_t -DC_TYPE=float16_t -V "$GSSRC" -o "$OUT/gemm_scalar_fp16_fp16.spv"
"$GLSLANG" $TENV -DA_TYPE=int8_t    -DC_TYPE=int32_t   -V "$GSSRC" -o "$OUT/gemm_scalar_s8_s32.spv"
# Vectorized FMA GEMM (vec4 coalesced loads + packed dot / dp4a).
V4SRC="$(dirname "$0")/shaders/fma_vec4.comp"
"$GLSLANG" $TENV -DA_TYPE=float16_t -DC_TYPE=float32_t -DVEC_TYPE=f16vec4          -V "$V4SRC" -o "$OUT/fma_vec4_fp16_fp32.spv"
"$GLSLANG" $TENV -DA_TYPE=float16_t -DC_TYPE=float16_t -DVEC_TYPE=f16vec4          -V "$V4SRC" -o "$OUT/fma_vec4_fp16_fp16.spv"
"$GLSLANG" $TENV -DA_TYPE=int8_t    -DC_TYPE=int32_t   -DVEC_TYPE=i8vec4 -DIS_INT=1 -V "$V4SRC" -o "$OUT/fma_vec4_s8_s32.spv"
# Inner-product FMA GEMM (dot per output) -- outer-vs-inner comparison.
INSRC="$(dirname "$0")/shaders/fma_inner.comp"
"$GLSLANG" $TENV -DA_TYPE=float16_t -DC_TYPE=float32_t -V "$INSRC" -o "$OUT/fma_inner_fp16_fp32.spv"
"$GLSLANG" $TENV -DA_TYPE=float16_t -DC_TYPE=float16_t -V "$INSRC" -o "$OUT/fma_inner_fp16_fp16.spv"
"$GLSLANG" $TENV -DA_TYPE=int8_t    -DC_TYPE=int32_t   -V "$INSRC" -o "$OUT/fma_inner_s8_s32.spv"
